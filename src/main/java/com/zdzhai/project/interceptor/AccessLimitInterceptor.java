...
public class AccessLimitInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        if (handler instanceof HandlerMethod) {
            ...
            if (maxLimit == null || "".equals(maxLimit)) {
                //set时一定要加过期时间
                stringRedisTemplate.opsForValue().set(key, "1", sec, TimeUnit.SECONDS);
            } else if (Integer.parseInt(maxLimit) < limit) {
                stringRedisTemplate.opsForValue().set(key, maxLimit + 1, sec, TimeUnit.SECONDS);
            } else {
