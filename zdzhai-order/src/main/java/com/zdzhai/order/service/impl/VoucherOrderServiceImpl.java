...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder>
    ...
    @Override
    public BaseResponse seckillVoucher(BuyVoucherOrderDTO buyVoucherOrderDTO, HttpServletRequest request) {
        //1.查询优惠券
        Long voucherId = buyVoucherOrderDTO.getId();
        Integer type = buyVoucherOrderDTO.getType();
        Long userId = buyVoucherOrderDTO.getUserId();
        ...
        User dbUser = apiOrderService.getLoginUser(request);
        if (dbUser == null) {
            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR);
        }
        if (!userId.equals(dbUser.getId())) {
...
//        ILock lock = new SimpleRedisLock("order:" + userId,stringRedisTemplate);
            ...
            try {
                //获取当前对象的代理对象
                proxy = (VoucherOrderService) AopContext.currentProxy();
                return proxy.createSeckillVoucherOrder(voucherId, userId);
            } finally {
                //释放锁
                lock.unlock();
            }
     ...
     * @return
     */
    private BaseResponse createVoucherOrder(Long voucherId, Long userId) {
        long count = this.query().eq("voucherId", voucherId)
                .eq("userId", userId).count();
        if (count > 0) {
            return ResultUtils.error(ErrorCode.SYSTEM_ERROR, "您已拥有该优惠券！");
        }
        //6.创建订单
    ...
    @Override
    @Transactional(rollbackFor = RuntimeException.class)
    public BaseResponse createSeckillVoucherOrder(Long voucherId, Long userId) {
        //一人一单
        //查询订单
        long count = this.query().eq("voucherId", voucherId)
                .eq("userId", userId).count();
